...
if(isset($_POST['submit'])){
    ...
    if($nombre==''||$apaterno==''||$amaterno==''||$user==''||$pass==''||$rol_id==''){
        ...
        header('Location:registrar.php?msg='.urlencode('El usuario ya existe'));
    }else{
        if($foto==''){
            $nomArchivo=null;
        }
                    ...
                    if(preg_match($nombres, $amaterno)){
                        if(preg_match($correos, $user)){
                            if(preg_match($contraseña, $pass)){
                                ...
                                header('Location:registrar.php?msg='.urlencode('La contraseña no tiene el formato correcto'));
                            }
                        }
                        else{
                            header('Location:registrar.php?msg='.urlencode('El correo no tiene el formato correcto'));
                        }
                    }
                    else{
                        header('Location:registrar.php?msg='.urlencode('El apellido materno no tiene el formato correcto'));
                    }
                }
                else{
                    header('Location:registrar.php?msg='.urlencode('El apellido parterno no tiene el formato correcto'));
                }
            }
            else{
                header('Location:registrar.php?msg='.urlencode('El nombre no tiene el formato correcto'));
            }
        }
...
<head>
    ...
    <link rel="Shortcut icon" type="image/x-icon" href="../asets/img/iconos/gota.png">
    ...
    <main class="container mt-5 fade-in">
        <div class="row bg-light rounded p-3">
            ...
            <div class="col-sm-12 col-md-12 col-lg-6">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" class="row g-3 p-3 rounded needs-validation" novalidate method="POST" enctype="multipart/form-data">
                    <?php
                        if(isset($_GET['msg'])){
